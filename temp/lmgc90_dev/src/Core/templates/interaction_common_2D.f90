...
function get_verlet_tact_lawnb(icdtac, iadj)
  ...
  character(len=103):: cout
   ...
   select case( cdmodel )
   case( i_rbdy2 )
     cdcol = get_color_RBDY2(verlt(icdtac)%cdbdy, verlt(icdtac)%cdtac)
   case( i_mbs2  )
     cdcol = get_color_MBS2D(verlt(icdtac)%cdbdy, verlt(icdtac)%cdtac)
   case( i_mailx )
   ...
   select case( anmodel )
   case( i_rbdy2 )
     ancol = get_color_RBDY2(verlt(icdtac)%anbdy(iadj), verlt(icdtac)%antac(iadj))
   case( i_mbs2  )
     ancol = get_color_MBS2D(verlt(icdtac)%anbdy(iadj), verlt(icdtac)%antac(iadj))
   case( i_mailx )
