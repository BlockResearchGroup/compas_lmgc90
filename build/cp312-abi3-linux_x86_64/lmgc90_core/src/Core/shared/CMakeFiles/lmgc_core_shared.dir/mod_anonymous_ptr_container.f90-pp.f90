# 1 "/home/pv/brg/code_fortran/compas_lmgc90/external/lmgc90_dev-fortran_lib/src/Core/shared/mod_anonymous_ptr_container.f90"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/home/pv/brg/code_fortran/compas_lmgc90/external/lmgc90_dev-fortran_lib/src/Core/shared/mod_anonymous_ptr_container.f90"
...
module anonymous_ptr_container
  ...
  public add_object_to_ptr_container, &
  ...
  subroutine add_object_to_ptr_container(objects, rank, c5, i4, r8, cx)
    implicit none
    type(ptr_container), intent(inout) :: objects !< [in,out] object container
    ...
    if( objects%open ) then
      ...
      call add_to_ptr_container(objects, object)
    else
      call faterr('anonymous_ptr_container::add_object_to_ptr_container','object container close')
    end if

  end subroutine

  !> \brief display an object container
  subroutine display_object_ptr_container(objects, ifich)
    implicit none
    type(ptr_container), intent(in) :: objects !< [in] object container
    ...
    call display_ptr_container(objects, ifich)
