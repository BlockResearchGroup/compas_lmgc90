...
! </read
       ...
       CALL set_time_step(TimeStep, .false.)
...
! read/>
       ...
       if (nb_SPHER == 0) call faterr("SPSP_standalone_DDM", "l'echantillon ne contient aucune sphere!")
...
! read/>
       ...
       call new_dd(nb_RBDY3, ntact_SPHER_=nb_SPHER)
      
       if (allocated(mask)) deallocate(mask)
       allocate(mask(nb_RBDY3), stat=ERR)
       if (err/=0) call faterr("SPSP_DDM", "erreur d'allocation de mask")
       !-------------------------------------------
       call stop_utimer(id_crea_dom)
       ! DDM/>
...
! free/>
         ...
         if (i == 1 .or. modulo(i, DDMfreq) == 0) then
             ...
             !am: le test "i > 1" est illicite puisque les donnees dans le repere de detection (localFrameTT) sont calculees au debut du pas et ne decoulent pas de la lecture... 
...
! </detect
          ...
          if ( check_SPSPx() ) then 
             ...
             !   !WRITE(*,'(1X,I10,A20)') get_nb_SPSPx(),' SPSPx roughly found'       
             !END IF
...
!</ updt
          ...
          ! affichage apres recollement
          if (MODULO(get_NSTEP(), freq_display) == 0) then 
             call gather_X_V
          end if
