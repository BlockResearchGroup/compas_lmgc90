...
program test_anonymous_ptr_container
  ...
  type(PTR_CONTAINER) :: objects ! the container
  ...
  call add_object_to_ptr_container(objects, 1, c5, i4, r8, cx)

  allocate(c5(1)); c5(1)   = 'blebl'
  allocate(i4(1)); i4(1)   = 1
  allocate(r8(2)); r8(1:2) = (/2.2D0, 1.1D0/)
  allocate(cx(3))
  call add_object_to_ptr_container(objects, 2, c5, i4, r8, cx)
  ...
  nullify(cx)
  call add_object_to_ptr_container(objects, 3, c5, i4, r8, cx)

  allocate(c5(1)); c5(1)   = 'blobl'
  allocate(i4(3)); i4(1:3) = (/1, 2, 3/)
  nullify(r8)
  allocate(cx(2)); cx(1)   = 'pas'; cx(2) = 'mieux'
  call add_object_to_ptr_container(objects, 4, c5, i4, r8, cx)

  print *, 'display content of container'
  call display_object_ptr_container(objects)

  print *, 'close container'
  call close_ptr_container(objects)

  print *, 'display content of container'
  call display_object_ptr_container(objects)

  print *, 'open container'
  call open_ptr_container(objects)

  print *, 'display content of container'
  call display_object_ptr_container(objects)

  print *, 'erasing container'
  call erase_ptr_container(objects)
  ...
  do i=1, 50
     ...
     do j=1, 20000
        call add_object_to_ptr_container(objects, j, c5, i4, r8, cx)
     end do
     ! delete it
     call erase_ptr_container(objects)
  end do
