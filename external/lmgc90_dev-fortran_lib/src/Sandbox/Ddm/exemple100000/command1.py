...
import time
#
sys.path.append('/home0/iceta0/LMGC90/LMGC90v2_dev/Sandbox/Ddm/python')
...
pas_temps = 0.005
max_iteration_step = 10000 # nombre iteration
max_time_step = 400 # nombre pas de temps
toleranceglobale  = 1.6666e-3
idim = 2       # la troisieme correpond aux rotations
theta = 0.5
arretlmgc = 0 # pour arreter le calcul avec les estimateurs de lmgc90 mettre =1
arretindicateurconv_rn = 1	# pour arreter le calcul avec l indicateur de convergence sur rn mettre =1
toleranceconv =1.0e-3  # valeur de test pour arret des iteration a convergence ( (ri(n) - rimoins1 /ri) = toleranceconv)
...
def Mise_en_donnee(numerosdm,file1):
	...
	for int1 in r_[0:nint1]:
		...
		if (shape(numer1)[1] > nbming1):
			...
			LocateGlobalStrongMicroNoeuds.append(nbnoeudsGMicro1)
			
			idimm = shape(ListMeshInt[int1])[1]*idof
			idimmnoeuds = shape(ListMeshInt[int1])[1]
			

			nddlGMicro1 = nddlGMicro1 + idimm
			nbnoeudsGMicro1 = nbnoeudsGMicro1 + idimmnoeuds
			...
			LocateGlobalStrongMicroNoeuds.append(0)
		#end
	...
	#opti sans matrices MSE,MapSdm1 = mM_Prepro_Sdm_Gran3(xcoort1,chpoE,nmail1)
	Liste_MSE,MapSdm1 = mM_Prepro_Sdm_Gran3_opti(xcoort1,chpoE,nmail1) #opti sans matrices 
	...
	#----------------------------------------------------------------------------------------------------------
	# reecriture complete simplifiee a base de listes
	
	
	liste_neud_sdm_Rg = asarray(numer1) 
	liste_neud_sdm_Rg = liste_neud_sdm_Rg.astype(int)
	#opti sans matrices print 'bE' ,myrank, shape(bE)
	#opti sans matrices print 'CE' ,myrank, shape(CE)
	#print 'bE.T' , bE.T
	#print 'CE',CE
	#opti sans matrices bet=mat(bE.T)
	#print 'bE.T * CE' , bet * CE
	#raw_input()
	#opti sans matrices AEG=bet * CE
	
	# tentative de creation de la liste des noeuds d'interface
	liste_neud_interface_Rg=[]
	liste_noeud_interface_sdm_Rg=[]
	for i23 in r_[0:len(ListMeshInt)]:
		#print 'test806' ,asarray(asarray(ListMeshInt[i23])[0]).astype(int)
		l2 = (asarray(asarray(ListMeshInt[i23])[0]).astype(int))
		liste_neud_interface_Rg= hstack([liste_neud_interface_Rg,l2]).astype(int)
		#print 'l1',l1
		#print 'liste',liste
		
		
		
		# ici bien fait interface par interface et dans meme ordre que CE sinon cest le gros bordel cf erreur de 4 jour avec be
		liste_complementaire = setdiff1d(liste_neud_sdm_Rg, l2) #element du sous domaine pas dans linterface
		#print 'liste_complementaire' ,liste_complementaire
		l3 = setdiff1d(liste_neud_sdm_Rg, liste_complementaire) #element du sous domaine pas dans (element du sous domaine pas dans linterface)
		liste_noeud_interface_sdm_Rg=hstack([liste_noeud_interface_sdm_Rg,l3]).astype(int)
	
	
	
	# ecriture de CE:
	
	#le plus beau mais voir ci dessous car faut aussi faire le signe (donne heureusement le meme resultat
	#CE_listenoeudsinterSDM_listenoeudsinterGlobaux = Findoccurences(liste_noeud_interface_sdm_Rg,liste_neud_interface_Rg)
		
	#tentative d ecriture de bet directement comme une liste locale des grains d interface
	bE_liste_noeud_interface_sdm_Rl = Findoccurences(liste_noeud_interface_sdm_Rg,liste_neud_sdm_Rg)
	
	
	
	#print 'bE_liste_noeud_interface_sdm_Rl',bE_liste_noeud_interface_sdm_Rl
	
	#raw_input()
	
	# ecriture de la matrice de signe
	...
	#opti sans matrices CE = zeros([0,nddlGMicro1])
	
	CE_listenoeudsinterSDM_listenoeudsinterGlobaux=[]
	#opti tout en noeuds CE_liste_nddlSDM_ddlGlobaux=[]
	CE_liste_signe=[]
	...
	for i1 in r_[0:len(ListInt1)]:
		...
		idimm = shape(ListMeshInt[int1-1])[1]#opti tout en noeuds *idof # %taille de l'interface
		...
		if (G1 <> 0):
			...
			localnoeud = r_[localind0:idimm+localind0]
			...
			globalnoeud = LocateGlobalStrongMicroNoeuds[int1-1]
			#globaldofs = globaldofs + [0:idimm-1];
			globalnoeud = globalnoeud + r_[0:idimm]
			
			
			#CE(localdofs,globaldofs) = sign1 * eye(idimm);
			#opti sans matrices CEinter=zeros([shape(localdofs)[0],nddlGMicro1])
			
			##print" 'nddlGMicro1',nddlGMicro1,'sdm1',sdm1
			
			for i1 in r_[0:len(localnoeud)]:
				
				##print" 'localdofs' ,localdofs,'globaldofs',globaldofs,sdm1
				i2=globalnoeud[i1]
				#opti sans matrices CEinter[i1,i2]=sign1
				
				CE_listenoeudsinterSDM_listenoeudsinterGlobaux=hstack([CE_listenoeudsinterSDM_listenoeudsinterGlobaux,i2])
				CE_liste_signe=hstack([CE_liste_signe,sign1])
	
	
	#verification qu'on obtien bien le meme resultat!!!
	#print 'CE_listenoeudsinterSDM_listenoeudsinterGlobaux',CE_listenoeudsinterSDM_listenoeudsinterGlobaux
	#print 'len(CE_liste_ddlSDM_ddlGlobaux)',len(CE_liste_ddlSDM_ddlGlobaux)
	#print 'CE_liste_signe',CE_liste_signe
	#print 'bE_liste_noeud_interface_sdm_Rl' ,bE_liste_noeud_interface_sdm_Rl
	
	#print 'len(CE_liste_signe)',len(CE_liste_signe)
	#print 'CE_listenoeudsinterSDM_listenoeudsinterGlobaux',CE_listenoeudsinterSDM_listenoeudsinterGlobaux
	#print 'len(CE_listenoeudsinterSDM_listenoeudsinterGlobaux)',len(CE_listenoeudsinterSDM_listenoeudsinterGlobaux)					
				
				#je regroupe les CE
				##print" 'i1',i1, sdm1
			#opti sans matrices CE=vstack([CE,CEinter])
		#end
	#end
	...
	#opti sans matrices bE,mapLocInt1 = Mm_Prepro_MapSdm(MapSdm1,ListMapInt2)
	...
	#opti sans matrices return chpoE,chmlE,mail1,nmail1,ListInt1,numer1,MSE,MapSdm1,ddlD1,UD1,ddlI1,DepiSdm,CE,bE,liste_neuds_total,ListMeshInt,CE_liste_ddlSDM_ddlGlobaux,CE_liste_signe,nddlGMicro1
	return liste_neud_sdm_Rg,Liste_MSE,liste_neuds_total,liste_noeud_interface_sdm_Rg,bE_liste_noeud_interface_sdm_Rl,nbnoeudsGMicro1,CE_listenoeudsinterSDM_listenoeudsinterGlobaux,CE_liste_signe,nbnoeudsGMicro1
...
def EtapeS2_solve_1_iteration_lmgc90():
	wrap_nlgs.exsolver('Stored_Delassus_Loops         ','Quad',toleranceglobale,1.,1,1)
...
def EtapeDD3_mise_en_donnee_du_sdm(myrank,file1):
	liste_neud_sdm_Rg,Liste_MSE,liste_neuds_total,liste_noeud_interface_sdm_Rg,bE_liste_noeud_interface_sdm_Rl,nbnoeudsGMicro1,CE_listenoeudsinterSDM_listenoeudsinterGlobaux,CE_liste_signe,nbnoeudsGMicro1 = Mise_en_donnee(myrank,file1)
	

	
	#print" 'AEG', AEG
	#opti sans matrices return CE,bE,liste_neud_sdm_Rg,AEG,MSE,liste_neuds_total,liste_noeud_interface_sdm_Rg,liste_noeud_interface_sdm_Rl
	return liste_neud_sdm_Rg,Liste_MSE,liste_neuds_total,liste_noeud_interface_sdm_Rg,bE_liste_noeud_interface_sdm_Rl,nbnoeudsGMicro1,CE_listenoeudsinterSDM_listenoeudsinterGlobaux,CE_liste_signe,nbnoeudsGMicro1

def Calcul_de_X(idof,bE_liste_noeud_interface_sdm_Rl,CE_listenoeudsinterSDM_listenoeudsinterGlobaux,CE_liste_signe,nbnoeudsGMicro1,Liste_MSE):
	
	#print 'MSE',MSE.XVAL
	
	#ListeMSEinv = linalg.inv(mat(MSE.XVAL))
	...
	intermed=zeros([nbnoeudsGMicro1*idof,nbnoeudsGMicro1*idof])
	
	nb_neud_interface_sdm = len(CE_listenoeudsinterSDM_listenoeudsinterGlobaux)
	
	#print 'nb_neud_interface_sdm',nb_neud_interface_sdm
	
	
	#print 'Liste_MSE',Liste_MSE
	
	#print 'idof',idof
	
	for i1 in r_[0:nb_neud_interface_sdm]:
		#print 'i1',i1
		numero_dans_interface_globale = CE_listenoeudsinterSDM_listenoeudsinterGlobaux[i1]
		#print 'numero_dans_interface_globale',numero_dans_interface_globale
		#print 'CE_listenoeudsinterSDM_listenoeudsinterGlobaux',CE_listenoeudsinterSDM_listenoeudsinterGlobaux
		valeur_signe_CE_interface = CE_liste_signe[i1]
		#print 'valeur_signe_CE_interface',valeur_signe_CE_interface
		numero_dans_sous_domaine = bE_liste_noeud_interface_sdm_Rl[i1]
		#print 'bE_liste_noeud_interface_sdm_Rl',bE_liste_noeud_interface_sdm_Rl
		#print 'numero_dans_sous_domaine',numero_dans_sous_domaine
		
		
		#dans la suite utiliser un get masse ou inertie
		# attention ne marche que car diagonal!!!!!
		for j in r_[0:idof]:
			
			#print 'j',j
			intermed[numero_dans_interface_globale*(idof)+j,numero_dans_interface_globale*(idof)+j]=1/Liste_MSE[numero_dans_sous_domaine*(idof)+j]*valeur_signe_CE_interface*valeur_signe_CE_interface
...
def EtapeDD4_set_visible(liste_neud_sdm_Rg,liste_neuds_total):
	
	
	#print 'liste_visible', liste_neud_sdm_Rg
	
	neud_caches = Difference_Listes(liste_neuds_total,liste_neud_sdm_Rg)
...
def getVector(nbDIME, liste_grains, idVect):
	...
	Vect=zeros((nbDIME+1)*nb_grains, 'd')
	# pour chaque grain
	for i in range(0, nb_grains):
		...
		# on vient placer les infos au bon endroi
		Vect[(nbDIME+1)*i:(nbDIME+1)*(i + 1)]=grainVect[0:(nbDIME+1)]
		#print 'test3000',nbDIME ,Vect[(nbDIME+1)*i:(nbDIME+1)*(i + 1)],grainVect
		#print 'test3001'
		# on renvoie le vecteur obtenu
	return Vect


def EtapeDD6_a_recup_VE(liste_noeud_interface_sdm_Rg,CE_listenoeudsinterSDM_listenoeudsinterGlobaux,bE_liste_noeud_interface_sdm_Rl,CE_liste_signe,liste_neud_sdm_Rg):
	...
	if (2==1):
		...
		VE3 = wrap_rbdy2.getvector('V____',liste_noeud_interface_sdm_Rg[0],3)
		##print" 'VE3',VE3
		...
		for i in liste_noeud_interface_sdm_Rg[r_[1:len(liste_noeud_interface_sdm_Rg)]]:
	...
	#nouvelle version optimisee:	on suppose ici que lon a pas besoin des vitesses des autres grain du sous domaine hors interface
	#on creer VE vide

	#print 'VE',VE
	#print 'liste_noeud_interface_sdm' ,liste_noeud_interface_sdm
	#print 'liste',liste
	#on trouve la position des grain d'interface parmis les autres
	#occur = Findoccurences(liste_noeud_interface_sdm,liste)	# trouve pour chacun des termes de val le premier indice ou il apparait dans list_val
	
	#print 'occur', occur
	
	if (2==2):
		nb_neud_interface_sdm = len(CE_listenoeudsinterSDM_listenoeudsinterGlobaux)
		nb_neud_sdm = len(liste_neud_sdm_Rg)
		VE=r_[0:(nb_neud_sdm)*idof]*0.0
		VE=asarray(VE)
	
		#print 'bE_liste_noeud_interface_sdm_Rl',bE_liste_noeud_interface_sdm_Rl
		#print 'nb_neud_interface_sdm',nb_neud_interface_sdm
		#print 'liste_noeud_interface_sdm_Rg',liste_noeud_interface_sdm_Rg
	
		#on parcourt les grains d interface	
		for i1 in r_[0:len(liste_noeud_interface_sdm_Rg)]:
			numero_corp_lmgc =  liste_noeud_interface_sdm_Rg[i1]
			#valeur_signe_CE_interface = CE_liste_signe[i1]
			numero_dans_sous_domaine = bE_liste_noeud_interface_sdm_Rl[i1]
			#print 'numero_dans_sous_domaine',numero_dans_sous_domaine
		
		
			#numero_corp_lmgc = liste_noeud_interface_sdm[i]
			#print 'numero_corp_lmgc',numero_corp_lmgc
			#position_corp_dans_sdm = occur[i]
			#print 'position_corp_dans_sdm',position_corp_dans_sdm
			#demande a lmgc90 uniquement les valeurs pour ces grains
			VE3 = wrap_rbdy2.getvector('V____',numero_corp_lmgc,3)
			#print 'VE3' , VE3[0:3]
			#print 'VE' , VE[numero_dans_sous_domaine*idof:(numero_dans_sous_domaine+1)*idof]
			for j in r_[0:idof]:
				VE[numero_dans_sous_domaine*idof+j]=VE3[j]
			#print 'VE' , VE[numero_dans_sous_domaine*idof:(numero_dans_sous_domaine+1)*idof]
...
def EtapeDD6_b_calcul_scdmbr_sdm(idof,nbnoeudsGMicro1,bE_liste_noeud_interface_sdm_Rl,CE_listenoeudsinterSDM_listenoeudsinterGlobaux,CE_liste_signe,VE):
	...
	scd_mbr = zeros([nbnoeudsGMicro1*idof,1])
	...
	for i1 in r_[0:nb_neud_interface_sdm]:
		...
		for j in r_[0:idof]:
			...
			scd_mbr[numero_dans_interface_globale*(idof)+j,0]=VE[numero_dans_sous_domaine*(idof)+j]*valeur_signe_CE_interface
			#print 'scd_mbr[numero_dans_interface_globale*(idof)+j,0]',scd_mbr[numero_dans_interface_globale*(idof)+j,0]
...
def EtapeDD6_d_calculFG(sum,X):
	...
	#print 'X',X,shape(X)
	#print 'scd_mbr',scd_mbr,shape(scd_mbr)
	
	DeltaFG = linalg.inv(X)*(sum);
	
	#print 'sum', sum,shape(sum)
	#print 'DeltaFG',DeltaFG,shape(DeltaFG)
...
def putVector(nbDIME, liste_grains, idVect, Vect):
  ...
  for i in range(0, nb_grains):
		...
		if nbDIME == 2:
			...
			grainVect[0:(nbDIME+1)]=Vect[(nbDIME+1)*i:(nbDIME+1)*(i+1)]
			# on envoie le vecteur contenant les donnees du grain courant
					
			#print 'test25000' , grainVect , Vect[(nbDIME+1)*i:(nbDIME+1)*(i+1)]
...
def EtapeM1_merger_donnees(liste_neuds_total,liste):
	...
	for ns in r_[0:nprocs]:
		...
		##print" 'min(ListetotaleAdj[ns])',min(ListetotaleAdj[ns]),'myrank',myrank,'ns',ns
...
def EtapeDD7_DISPATCHE_FG(liste_noeud_interface_sdm_Rg,nbnoeudsGMicro1,bE_liste_noeud_interface_sdm_Rl,CE_listenoeudsinterSDM_listenoeudsinterGlobaux,CE_liste_signe,DeltaFG,idof,pas_temps):
		...
		#for i in r_[0:len(DeltaFG)/(nbDIME+1)]: #car lmgc travail ni en impultion ni en force...
		#	DeltaFG[i*(nbDIME+1)]= -DeltaFG[i*(nbDIME+1)]/pas_temps #gonflage
		#	DeltaFG[i*(nbDIME+1)+1]= -DeltaFG[i*(nbDIME+1)+1]/pas_temps #gonflage
		#	DeltaFG[i*(nbDIME+1)+2]= -DeltaFG[i*(nbDIME+1)+2]/pas_temps  #gonflage
		
		
		
		
		
		DeltaFG = -DeltaFG/pas_temps
		...
		for i1 in r_[0:nb_neud_interface_sdm]:
			...
			numero_corp_lmgc =  liste_noeud_interface_sdm_Rg[i1]
			#numero_corp_lmgc =  CE_listenoeudsinterSDM_listenoeudsinterGlobaux[i1]+1
			valeur_signe_CE_interface = CE_liste_signe[i1]
			numero_dans_sous_domaine = bE_liste_noeud_interface_sdm_Rl[i1]
			position_interface_globale = CE_listenoeudsinterSDM_listenoeudsinterGlobaux[i1]
			...
			for j in r_[0:idof]:
				#print 'idof*i1+j',idof*i1+j
				#print 'shape(DeltaFG)',shape(DeltaFG)
				grainVect[j]=DeltaFG[idof*position_interface_globale+j,0]*valeur_signe_CE_interface
				
			#print 'FE[position_corp_dans_sdm*idof:(position_corp_dans_sdm+1)*idof]',FE[position_corp_dans_sdm*idof:(position_corp_dans_sdm+1)*idof].T[0]
			#print 'grainVect[0:3]',grainVect[0:3]
			#grainVect[0:3]=FE[position_corp_dans_sdm*idof:(position_corp_dans_sdm+1)*idof]
		
			#rentrer les valeur dans lmgc
			wrap_rbdy2.putvector('Fext_',numero_corp_lmgc,grainVect)
		...
		#	normcarree=dot(grainVect,grainVect)
		#	
		#	if (normcarree > 1.e-15): #optimisation basee sur l utilisation des matrices (on ne fait rien pour les grain hors du bord)
		...
		#
		##on parcourt les grains d interface
		#for i in r_[0:len(liste_noeud_interface_sdm)]:
		#
		#	numero_corp_lmgc = liste_noeud_interface_sdm[i]
		#	#print 'numero_corp_lmgc',numero_corp_lmgc
		#	position_corp_dans_sdm = occur[i]
		...
		#	grainVect=zeros(3, 'd')
		#	for j in r_[0:idof]:
		#		grainVect[j]=FE[idof*position_corp_dans_sdm+j,0]
		#	#print 'FE[position_corp_dans_sdm*idof:(position_corp_dans_sdm+1)*idof]',FE[position_corp_dans_sdm*idof:(position_corp_dans_sdm+1)*idof].T[0]
		#	#print 'grainVect[0:3]',grainVect[0:3]
		#	#grainVect[0:3]=FE[position_corp_dans_sdm*idof:(position_corp_dans_sdm+1)*idof]
		#
...
#stockage des iterations a convergence de chaque sous domaine
name='nbiterconv.SDMnumero.txt'
name= name.replace('numero',str(myrank))
filenbiterconv = open(name,'w')
...
if (myrank ==0):
	...
	name='Solver_information_ddm_indic_conv.txt'
	filenSolverinfoddm_indicateur_conv = open(name,'w')
	filenSolverinfoddm_indicateur_conv.write("iteration_step"+ " ; " +"Total_QuadDV" + " ; " +"Total_QuadDVR" + " ; " +"Total_MeanDVoR")
...
# boucle sur le temps
for time_step in r_[1:max_time_step]:
	...
	name='indicateur.SDMnumero.STEPpas.txt'
	name= name.replace('numero',str(myrank))
	name= name.replace('pas',str(time_step))
	fileindicateurstep = open(name,'w')
	
	chaine=("\n" + "iteration_step"+ " ; " + "SdmQuadDV" + " ; " + "SdmQuadDVR"+ " ; " + "SdmMeanDVoR" )
	filerreurstep.write(chaine)
	chaine=("\n" + "iteration_step"+ " ; " + "indicateur_conv_rn" + " ; " + "indicateur_conv_rt"+ " ; " + "indicateur_conv_r" )
	fileindicateurstep.write(chaine)
	...
	liste_neud_sdm_Rg,Liste_MSE,liste_neuds_total,liste_noeud_interface_sdm_Rg,bE_liste_noeud_interface_sdm_Rl,nbnoeudsGMicro1,CE_listenoeudsinterSDM_listenoeudsinterGlobaux,CE_liste_signe,nbnoeudsGMicro1 = EtapeDD3_mise_en_donnee_du_sdm(myrank,file1)
	stopCPU,stopHM = TimingStop()
	filout.write("\nDD3 ; " + " Temps CPU ; " + str((stopCPU - startCPU)*1000.)+ "; Temps Machine Start; " + str(startHM)+ "; Temps Machine Stop; " + str(stopHM))
	
	startCPU,startHM = TimingStart()
	X=Calcul_de_X(idof,bE_liste_noeud_interface_sdm_Rl,CE_listenoeudsinterSDM_listenoeudsinterGlobaux,CE_liste_signe,nbnoeudsGMicro1,Liste_MSE)
	stopCPU,stopHM = TimingStop()
	filout.write("\nCalculX ; " + " Temps CPU ; " + str((stopCPU - startCPU)*1000.)+ "; Temps Machine Start; " + str(startHM)+ "; Temps Machine Stop; " + str(stopHM))
	
	startCPU,startHM = TimingStart()
	EtapeDD4_set_visible(liste_neud_sdm_Rg,liste_neuds_total)
	stopCPU,stopHM = TimingStop()
	...
	for iteration_step in r_[1:max_iteration_step]:
		...
		startCPU,startHM = TimingStart()
		EtapeDD7_DISPATCHE_FG(liste_noeud_interface_sdm_Rg,nbnoeudsGMicro1,bE_liste_noeud_interface_sdm_Rl,CE_listenoeudsinterSDM_listenoeudsinterGlobaux,CE_liste_signe,DeltaFG,idof,pas_temps)
		stopCPU,stopHM = TimingStop()		
		...
		startCPU,startHM = TimingStart()
		####################################   ECRITURE ET CALCUL D ERREUR #########################################################
		#sortir une valeur de l'erreur
		DSumDVDVRR,DNactif,DMeanWRR, Dtol,DSumDVDV,DQuadWR,DSumDVoR,DSumWRR  = wrap_nlgs.geterror()
		
		sum_t,sum_n,dsum_t,dsum_n = wrap_nlgs.getconv()
		#print 'sum_t,sum_n,dsum_t,dsum_n',sum_t,sum_n,dsum_t,dsum_n
		
		indicateur_convergence_rn = dsum_n/sum_n
		indicateur_convergence_rt = dsum_t/sum_t
		indicateur_convergence_r = (dsum_n**2+dsum_t**2)**0.5/(sum_n**2+sum_t**2)**0.5
		
		#Somme par MPI:
		Total_sum_t = MPI.COMM_WORLD.Allreduce(sum_t,MPI.SUM)
		Total_dsum_t = MPI.COMM_WORLD.Allreduce(dsum_t,MPI.SUM)
		Total_sum_n = MPI.COMM_WORLD.Allreduce(sum_n,MPI.SUM)
		Total_dsum_n = MPI.COMM_WORLD.Allreduce(dsum_n,MPI.SUM)
		
		Total_indicateur_convergence_rn = Total_dsum_n/Total_sum_n
		Total_indicateur_convergence_rt = Total_dsum_t/Total_sum_t
		Total_indicateur_convergence_r = (Total_dsum_n**2+Total_dsum_t**2)**0.5/(Total_sum_n**2+Total_sum_t**2)**0.5
		...
		filerreurstep.write(chaine)
		chaine=("\n" + str(iteration_step)+ " ; " +str(indicateur_convergence_rn) + " ; " +str(indicateur_convergence_rt) + " ; " +str(indicateur_convergence_r))
		fileindicateurstep.write(chaine)
		 		

		#stockage du nombre d interation pour le sous domaine en  question avant de passer en dessous de la barre de teste
		if (SdmQuadDV < toleranceglobale) and (SdmQuadDVR < toleranceglobale) and (SdmMeanDVoR < toleranceglobale):
			chaine=("\n Time_step; "+ str(time_step)+" ; nb_iterations;"+ str(iteration_step)+ " ; " +str(SdmQuadDV) + " ; " +str(SdmQuadDVR) + " ; " +str(SdmMeanDVoR ))
			filenbiter.write(chaine)
		
		#stockage du nombre d interation pour le sous domaine en  question avant de passer en dessous de la barre de teste
		if (indicateur_convergence_rn < toleranceconv):
			chaine=("\n Time_step; "+ str(time_step)+" ; nb_iterations ; "+ str(iteration_step)+ " ; " +str(indicateur_convergence_rn) + " ; " +str(indicateur_convergence_rt) + " ; " +str(indicateur_convergence_r ))
			filenbiterconv.write(chaine)
		####################################   ECRITURE ET CALCUL D ERREUR #########################################################
		stopCPU,stopHM = TimingStop()
		filout.write("\nCAE; " + " Temps CPU ; " + str((stopCPU - startCPU)*1000.)+ "; Temps Machine Start; " + str(startHM)+ "; Temps Machine Stop; " + str(stopHM))
		...
		startCPU,startHM = TimingStart()
		VE=EtapeDD6_a_recup_VE(liste_noeud_interface_sdm_Rg,CE_listenoeudsinterSDM_listenoeudsinterGlobaux,bE_liste_noeud_interface_sdm_Rl,CE_liste_signe,liste_neud_sdm_Rg)
		stopCPU,stopHM = TimingStop()
		...
		startCPU,startHM = TimingStart()
		scd_mbr=EtapeDD6_b_calcul_scdmbr_sdm(idof,nbnoeudsGMicro1,bE_liste_noeud_interface_sdm_Rl,CE_listenoeudsinterSDM_listenoeudsinterGlobaux,CE_liste_signe,VE)
		stopCPU,stopHM = TimingStop()
		...
		# pour arreter le calcul avec les estimateurs de lmgc90
		if (arretlmgc == 1):
			if (Total_QuadDV < toleranceglobale) and (Total_QuadDVR < toleranceglobale) and (Total_MeanDVoR < toleranceglobale):
		...
		# pour arreter le calcul avec l indicateur de convergence sur rn
		if (arretindicateurconv_rn == 1):
			if (Total_indicateur_convergence_rn < toleranceconv) :
				print 'erreur admissible ok', 'rank',myrank
				if (myrank ==0):
					# seul 0 ecrit
					filenSolverinfoddm.write(str(iteration_step)+ " ; " +str(Total_QuadDV) + " ; " +str(Total_QuadDVR) + " ; " +str(Total_MeanDVoR ))
					filenSolverinfoddm_indicateur_conv.write(str(iteration_step)+ " ; " +str(Total_indicateur_convergence_rn) + " ; " +str(Total_indicateur_convergence_rt) + " ; " +str(Total_indicateur_convergence_r ))
				# Tout le monde fait le break
				break
				#tester la valeur de l'erreur pour arreter la boucle
	...
	fileindicateurstep.close() # Ferme le fichier
	...
	startCPU,startHM = TimingStart()
	EtapeM1_merger_donnees(liste_neuds_total,liste_neud_sdm_Rg)
	stopCPU,stopHM = TimingStop()
	...
	if (myrank==0):
		...
		if (floor(time_step*1./pas_ecriture_gmv)*10.-(time_step*1./pas_ecriture_gmv)*10.==0):
			...
			wrap_post2d.writeoutgmv(1)
                wrap_overall.writeoutdof(1)
                wrap_rbdy2.writeoutdof()
                wrap_overall.writeoutvlocrloc(1)
		wrap_dkdkx.writeoutvlocrloc()
   
   
filenbiter.close()
filenbiterconv.close()

filout.close() # Ferme le fichier
if (myrank ==0):
	filenSolverinfoddm.close()
	filenSolverinfoddm_indicateur_conv.close()
